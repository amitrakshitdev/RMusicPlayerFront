...
export const playlistSlice = createSlice({
  ...
  reducers: {
    ...
    playNext: (state, action: PayloadAction<Song>) => {
      ...
      const existingIndex = state.currentPlaylist.findIndex((song) => song.videoId === newSong.videoId);
    ...
    changePlaylist: (state, action: PayloadAction<Song[]>) => {
      ...
      state.currentSongId = action.payload.length > 0 ? action.payload[0].videoId : null;
    ...
    playAtIndex: (state, action: PayloadAction<number>) => {
      if (action.payload >= 0 && action.payload < state.currentPlaylist.length) {
        state.currentPlayingIndex = action.payload;
        state.currentSongId = state.currentPlaylist[action.payload].videoId;
        state.isPlaying = true;
      }
    },
    nextSong: (state) => {
      if (state.currentPlaylist.length > 0 && state.currentPlayingIndex !== null) {
        const nextIndex = (state.currentPlayingIndex + 1) % state.currentPlaylist.length;
    ...
    prevSong: (state) => {
      if (state.currentPlaylist.length > 0 && state.currentPlayingIndex !== null) {
        const prevIndex = (state.currentPlayingIndex - 1 + state.currentPlaylist.length) % state.currentPlaylist.length;
    ...
    removeSongFromQueue: (state, action: PayloadAction<string>) => {
      ...
      const indexToRemove = state.currentPlaylist.findIndex((song) => song.videoId === songIdToRemove);
      ...
      if (indexToRemove !== -1) {
        ...
        if (state.currentPlayingIndex !== null && indexToRemove < state.currentPlayingIndex) {
          ...
          state.currentPlayingIndex = state.currentPlaylist.length > 0 ? Math.min(indexToRemove, state.currentPlaylist.length - 1) : null;
          state.currentSongId = state.currentPlayingIndex !== null ? state.currentPlaylist[state.currentPlayingIndex].videoId : null;
          state.isPlaying = state.currentPlaylist.length > 0 && state.currentPlayingIndex !== null;
        }
      }
    },
    play: (state) => {
...
export const selectCurrentPlayingIndex = (state: { playlist: PlaylistState }) => state.playlist.currentPlayingIndex;
export const selectCurrentSongId = (state: { playlist: PlaylistState }) => state.playlist.currentSongId;
export const selectIsPlaying = (state: { playlist: PlaylistState }) => state.playlist.isPlaying;
export const selectCurrentPlayingSong = (state: { playlist: PlaylistState }) => {
  const currentIndex = state.playlist.currentPlayingIndex;
  return currentIndex !== null && state.playlist.currentPlaylist.length > currentIndex
